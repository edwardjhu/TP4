...
from meta.maml.utils import tensors_to_device
...
class CachedOmniglot(CombinationMetaDataset):
    def __init__(self, root, num_classes_per_task=None, meta_train=False,
        ...
        dataset = CachedOmniglotClassDataset(root, meta_train=meta_train,
...
class OmniglotDataset(Dataset):
    ...
    #     print(transform)
    #     print('before', self.transform)
    #     super().target_transform_append(transform)
    #     print('after', self.transform)

#### end copy

class CachedOmniglotClassDataset(OmniglotClassDataset):
    ...
    def __getitem__(self, index):
        ...
        # self.cache[index] = OmniglotDataset(index, data, character_name,
        #     transform=transform, target_transform=target_transform)
        
        # return self.cache[index]
        return CachedOmniglotDataset(index, data, character_name,
            transform=transform, target_transform=target_transform,
            cache=self.cache[index])


class CachedOmniglotDataset(OmniglotDataset):
    def __init__(self, *args, **kw):
    ...
    def __getitem__(self, index):
        if index in self.cache:
            ...
            if self.transform is not None:
                image = self.transform(image)
            self.cache[index] = tensors_to_device(image)
        ...
        # import pdb; pdb.set_trace()
        # self.cache[index] = (tensors_to_device(image), target)
